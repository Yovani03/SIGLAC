...
const EquiposDocente = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            showToast("Ítem registrado con éxito.");
            setNewItem({
    ...
    const handleDeleteItem = async (type, itemId) => {
        if (!window.confirm(`¿Está seguro de eliminar permanentemente este ítem (${itemId})? Esta acción no se puede deshacer.`)) return;
    ...
    if (loading) return <div className="p-10 text-center text-slate-500 font-bold uppercase tracking-widest">Cargando datos de sus laboratorios...</div>;
    ...
    return (
        <div className="space-y-6 animate-fadeIn pb-20">
            ...
            <div className="flex space-x-1 bg-slate-100 p-1.5 rounded-2xl w-fit">
                ...
                >
                    <Plus className="w-4 h-4 mr-2" /> Alta de Equipo
                </button>
                <button
                                        ...
                                        {isPC() && (
                                            <div className="p-6 bg-amber-50 rounded-[2rem] space-y-4 border border-amber-100">
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    <input type="text" placeholder="RAM" className="px-5 py-3 bg-white border border-amber-100 rounded-xl text-sm font-bold" value={newItem.ram} onChange={(e) => setNewItem({ ...newItem, ram: e.target.value })} />
                                                    <input type="text" placeholder="Almacenm." className="px-5 py-3 bg-white border border-amber-100 rounded-xl text-sm font-bold" value={newItem.disco_duro} onChange={(e) => setNewItem({ ...newItem, disco_duro: e.target.value })} />
                                                </div>
                                            </div>
                                        )}
